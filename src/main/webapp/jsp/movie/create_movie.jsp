<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html>
<head>
	<meta charset="ISO-8859-1">
	<title>NNMOVIE</title>
	<link rel = "icon" href = 
			"data:image/jpeg;base64," 
	        type = "image/x-icon">
	<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.1/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-+0n0xVW2eSR5OomGNYDnhzAbDsOXxcvSN1TPprVMTNDbiYZCxYbOOl7+AMvyTG2x" crossorigin="anonymous">
	<link rel="stylesheet" href="css/create_movie.css">
</head>
<body>
	<div id="header">
		<jsp:include page="admin_header.jsp" />
	</div>
	
	<div class="container-fullwidth">
		<div class="row">
			<div class="movie col-md-6 col-md-offset-3">
				<h1 class="well text-center">CREATE MOVIE</h1>
				<div class="col-lg-12 well">
					<div class="row">
						<form action="register_success_movie.jsp" method="post">
							<div class="col-sm-12">
								
									<div class=" form-group">
										<label class="required">Name</label><input type="text"
											placeholder="Enter Movie Name Here.." class="form-control"
											name="name" required>
									</div>
									
									<div class=" form-group">
										<label class="required">Image</label><input type="text"
											placeholder="Enter Image Source URL Here..." class="form-control"
											name="image" required>
									</div>
									<div class=" form-group">
										<label class="required">Description</label> <input type="text"
											placeholder="Enter Movie Description Here.." class="form-control"
											name="description" required>
									</div>
								

								<div class=" form-group">
									<label class="required">Publishing Year:</label> <input type="date"
										 class="form-control"
										name="pub_year" required>
								</div>
							
								<div class=" form-group">
										<label class="required">Type</label>
										<select class="custom-select mr-sm-2" id="inlineFormCustomSelect">
									        <option selected>Fantastic</option>
									        <option value="1">Drama</option>
									        <option value="2">Horror</option>
									        <option value="3">Romance</option>
									        <option value="4">Western</option>
									        <option value="5">Comedy</option>
									        <option value="6">Thriller</option>
									        <option value="7">Documentary</option>
									        <option value="8">Animation</option>
									        <option value="9">Musical</option>
									    </select>
								</div>
								
								<div class=" form-group">
										<label class="required">Language</label> <input type="text"
											placeholder="Enter Movie Language Here.." class="form-control"
											name="description" required>
								</div>
								
								<div class=" form-group">
										<label class="required">Actor/Actrees</label><input type="text"
											placeholder="Enter Actor/Actrees Name And Surname In The Movie.." class="form-control"
											name="actor_actrees" required>
								</div>	
								<input type="submit" value="Submit" class="btn btn-lg btn-info">
								</input>
								<input type="button" value="Back" class="btn btn-lg btn-info" onclick ="location.href='admin_movie_home.jsp';"
								style="margin-left:400px">
								</input>
							</div>
						</form>
					</div>
				</div>
			</div>
		</div>

		</div>
		<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.1/dist/js/bootstrap.bundle.min.js" integrity="sha384-gtEjrD/SeCtmISkJkNUaaKMoLD0//ElJ19smozuHV6z3Iehds+3Ulb9Bn9Plx0x4" crossorigin="anonymous"></script>
		
</body>
</html>